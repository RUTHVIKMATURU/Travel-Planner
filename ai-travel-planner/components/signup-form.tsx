...
export function SignupForm() {
  ...
  const onSubmit = async (data: SignupInput) => {
    ...
    try {
      ...
      // This keeps the button in a loading state while Next.js fetches and renders the login page.
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col space-y-2 pt-2">
          <div className="flex items-center space-x-2">
            <Checkbox 
              ...
              disabled={isLoading}
            />
            <Label htmlFor="terms" className="text-sm font-medium text-zinc-600 dark:text-zinc-400 leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
              I accept the <a href="#" className="font-semibold text-zinc-900 dark:text-zinc-100 hover:text-primary transition-colors hover:underline">terms and conditions</a>
            </Label>
          </div>
          {errors.terms && <p className="text-sm text-destructive font-medium">{errors.terms.message}</p>}
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button variant="outline" className="w-full bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 font-medium shadow-sm active:scale-[0.98] transition-all" disabled={isLoading} asChild>
